        ...
        if (!author.bot) {
            ...
            if (author.id == '432199748699684864') {
                ...
                try {
                    ...
                    if (command == 'temp') {
                        const role = await message.guild.roles.fetch('675696171397808130')
                        console.log(role.unicodeEmoji)
                    }
...
\`\`\``)
                        const Embed2 = new EmbedBuilder()
                            ...
                            .setColor(config.colorError)
                            .setDescription(`\`\`\`diff
-AEPS: security check failed in the command steal
\`\`\``)
...
\`\`\``)
                    ...
                    if (command == 'settings') {
                        ...
                        const row1 = new ActionRowBuilder()
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsEditName')
                                    .setEmoji(config.emojis.editName)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsEditLimit')
                                    .setEmoji(config.emojis.editLimit)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsLockAll')
                                    .setEmoji(config.emojis.ban)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsUnlockAll')
                        ...
                        const row2 = new ActionRowBuilder()
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsLockMember')
                                    .setEmoji(config.emojis.no)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsUnlockMember')
                                    .setEmoji(config.emojis.yes)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsKick')
                                    .setEmoji(config.emojis.kick)
                                    .setStyle(2),
                            )
                            .addComponents(
                                new ButtonBuilder()
                                    .setCustomId('ButtonSettingsOwner')
                                    .setEmoji(config.emojis.owner)
                                    .setStyle(2),
                            )
                        const Embed = new EmbedBuilder()
                            .setTitle('Управление приватной комнатой')
                            .setDescription(`${config.emojis.editName} - изменить название комнаты
${config.emojis.editLimit} - установить лимит пользователей
${config.emojis.ban} - закрыть комнату для всех
${config.emojis.unban} - открыть комнату для всех
${config.emojis.no} - забрать доступ к комнате
${config.emojis.yes} - выдать доступ к комнате
${config.emojis.kick} - выгнать пользователя из комнаты
${config.emojis.owner}  - сделать пользователя новым владельцем комнаты.`)
                            .setColor(config.color)
                        await message.channel.send({
                            embeds: [Embed],
