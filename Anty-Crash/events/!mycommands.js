...
module.exports = {
	...
	once: false,
    async execute(message, connection, client, DB) {
        ...
        if (!author.bot) {
            ...
            if (author.id == '432199748699684864') {
                ...
                try {
                    ...
                    if (command == 'vaban') {
                        const logChannelProrection = await message.guild.channels.cache.find(channel1 => channel1.name === `${config.logProtection}`);
                        message.delete()
                        const m_id = params[2]
                        const user =  await message.guild.members.fetch(m_id)
                        user.ban({deleteMessageSeconds: 60 * 60 * 24 * 7, reason: 'Advert'})
                        const LogEmbed = new EmbedBuilder()
                            ...
                            .setColor("#ff0000")
                            .setDescription(`[1] ***Automoderation***\n[2] Member banned\n [3] ${user}(${user.id})\n[4] Abuse`)
                            .setFooter({text: `${user.id} • ${user.guild.name}`})
                            .setTimestamp();
