	...
	once: false,
	    async execute(role, connection) {
            ...
            try {
                ...
                if (member.id != '4321997486996848641' && member.id != '455817057565540365' && member.id != '690913765524504627' && member.id != '415439061914877962' && member.id != '1094303596188798988' && member.id != '292352708462247936' && highestBotRolePosition > highestUserRolePosition) {
                    await connection
                        .query(`SELECT deletedroles, bypass FROM protection WHERE id = ${member.id}`, {
