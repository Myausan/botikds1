    ...
    async execute(interaction, connection, lockedCommands) {
        ...
        try {
            const rowAfisha = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        .setCustomId('buttonKaraokeSend')
                        .setLabel('Отправить')
                        .setStyle(3),
                )
                .addComponents(
                    new ButtonBuilder()
                        .setCustomId('buttonKaraokeCancel')
                        .setLabel('Отмена')
                        .setStyle(4),
                )
            ...
            const embed = new EmbedBuilder()
                .setTitle('⠀⠀⠀⠀⠀⠀⠀ ⠀ ⠀ ⠀⠀⠀⠀⠀ ⠀⠀⠀⠀⠀Караоке')
                .setDescription(`**Здравствуйте дорогие участники сервера!**\n\n<:Line:1157746076904857660>  **${date.getDay()}.${date.getMonth()}.${date.getFullYear()} в ${date.getHours()}:${date.getMinutes()} по МСК**, на нашем сервере будет проходить мероприятие **«Караоке»**.\n\n<:Line:1157746076904857660> **Формат проведения:**\n\`\`\`Карао́ке — развлечение, заключающееся в непрофессиональном пении с использованием электронного устройства или без него, позволяющего петь под заранее записанную музыку; также само это устройство. Вопрос использования фонограмм авторов и исполнителей по сей день остаётся спорным и открытым.\`\`\`\n<:Line:1157746076904857660> **Ведущие: <@${member1.id}> & <@${member2.id}>**\n\n**__Ждем вас на нашем мероприятии!__**`)
            ...
            collector.on('collect', async ButtonInteraction => {
                ...
                collector.stop()
                if (ButtonInteraction.customId === 'buttonKaraokeSend') {
                    await afishaChannel.send({
                        //content: `<@&1111402486570418217>`,
                        embeds: [embed]
                    }) 
                    confirmEmbed.setDescription(`${author}, ебхук отправлен`).setColor('#00ff00')
                    await interaction.editReply({
                        embeds: [embed, confirmEmbed],
                        components: []
                    })
                }
                if (ButtonInteraction.customId === 'buttonKaraokeCancel') {
