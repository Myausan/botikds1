...
module.exports = {
    ...
    async execute(interaction, connection, lockedCommands) {
        ...
        const author = interaction.memberы  
        let cancel = false
        let image
        let message
            ...
            collector.on('collect', async ButtonInteraction => {
                ...
                if (ButtonInteraction.customId === 'buttonEmbedImage') {
                    const imageEmbed = new EmbedBuilder()
                        ...
                        .setThumbnail(author.user.displayAvatarURL())
                        .setColor(config.color)
                        .setDescription(`${author}, отправьте 1 файл`)
                    await ButtonInteraction.reply({
                        embeds: [imageEmbed],
                        ephemeral: true
                    }) 
                    const filter = messageEmbedImage => messageEmbedImage.author.id === author.id;

                    const collector1 = interaction.channel.createMessageCollector({filter, time: 60000 });

                    collector1.on('collect', async messageEmbedImage => {
                        collector1.stop()
                        if (messageEmbedImage.attachments.size != 1) {
                            const errorEmbed = new EmbedBuilder()
                                .setTitle(title)
                                .setThumbnail(author.user.displayAvatarURL())
                                .setColor(config.colorError)
                                .setDescription(`${author}, вам нужно прикрепить 1 файл`)
                            await messageEmbedImage.reply({
                                embeds: [errorEmbed]
                            }) 
                            return 
                        }
                        for (let [key, value] of messageEmbedImage.attachments) {
