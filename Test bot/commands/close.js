...
module.exports = {
    ...
    async execute(interaction, connection, DB) {
        ...
        switch (subcommand) {
            ...
            case 'action': title=`Отменить создание клоуза`;break;
        }
        if (!author.roles.cache.has(config.roleEvent) && author.id != config.owner_id) {
        ...
        if (subcommand === 'create') {
            ...
            collector.on('collect', async ButtonInteraction => {
                ...
                .then(async ModalInteraction => {
                    ...
                    let timestampInput = ModalInteraction.components[0].components[0].value
                    let time = new Date(timestampInput-2*60*60*1000)
                    let timeEmbed = `${time.getHours()}:${time.getMinutes()}`
                    await connection
                    ...
                    if (event) {
                        const errorEmbed = new EmbedBuilder()
                            .setTitle(title)
                            .setColor(config.colorError)
                            .setDescription(`${author}, у вас уже есть активный клоуз`);
                        await interaction.editReply({
                            embeds: [errorEmbed],
                            components: []
                        })
                    ...
                    if (eventtime > Date.now() && !bypass1) {
                        ...
                        const errorEmbed = new EmbedBuilder()
                            .setTitle(title)
                            .setColor(config.colorError)
                            .setDescription(`${author}, вы уже недавно создавали клоуз. Создать следующий можно через: ${result}`);
                        await interaction.editReply({
                            embeds: [errorEmbed],
                            components: []
                        })
                    ...
                    if (!((timestampInput == parseInt(timestampInput)) && (parseInt(timestampInput)*1000 > Date.now()) )) {
                        const errorEmbed = new EmbedBuilder()
                            .setTitle(title)
                            .setColor(config.colorError)
                    ...
                    const EmbedEvent = new EmbedBuilder()
                        .setTitle(eventText.embeds[0].title)
                        .setDescription(eventText.embeds[0].description)
                        .setColor(eventText.embeds[0].color)
                        .addFields(eventText.embeds[0].fields)
                        .setImage(eventText.embeds[0].image.url)
