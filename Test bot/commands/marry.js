	...
	async execute(interaction, connection, DB) {
        ...
        try {
            ...
            if (sqlResult[0] === undefined) {
                ...
                a_exp = sqlResult[0].exp;
                a_baneconomy = sqlResult[0].baneconomy;
                a_ghost = sqlResult[0].ghost;
            }
            await connection
                .query(`SELECT exp, baneconomy FROM money WHERE id = ${member.id}`, {
                    type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
            })
                .then((result) => sqlResult = result)
            if (sqlResult[0] === undefined) {
            ...
            if (a_balance < 2000) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(author.user.displayAvatarURL())
                    .setColor(config.colorError)
                    .setDescription(`${member}, для вступления в брак, нужно купить кольца, их стоимость 2000${emoji}\n\nВаш баланс: ${a_balance}${emoji}`);
                await interaction.reply({
                    embeds: [errorEmbed]
                })
                return
            ...
            if (a_level < 10) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(author.user.displayAvatarURL())
                    .setColor(config.colorError)
                    .setDescription(`${author}, для заключения брака нужен 10 уровень`);
                await interaction.reply({
                    embeds: [errorEmbed]
                })
                return
            ...
            if (m_level < 10) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(author.user.displayAvatarURL())
                    .setColor(config.colorError)
                    .setDescription(`${author}, у ${member} нет 10 уровня`);
                await interaction.reply({
                    embeds: [errorEmbed]
                })
                return
            ...
            const Embed = new EmbedBuilder()
                ...
                .setDescription(`${author}, для покупки колец нужно 1000 ${emoji}, вы уверены что хотите совершить данную операцию?`)
            await interaction.reply({
                embeds: [Embed],
                components: [row],
                fetchReply: true
