	...
	once: true,
	    async execute(client, connection) {
            ...
            while (true) {
                ...
                await connection
					.query(`SELECT COUNT(*) as count FROM tmmembers WHERE timestamp < ${now};`, {
						type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
					})
                .then((result) => sqlResult = result)
                .catch((err) => {
                ...
                if (sqlResult[0].count != 0) {
                    ...
                    .then((result) => sqlResult = result)
                    for (let i = 0; i < count; i++) {
                        let memberId = sqlResult[i].id
                        roleid = sqlResult[i].roleid
                        const member = await guild.members.fetch(memberId)
                        const role = await guild.roles.fetch(roleid)
                        await connection
                            .query(`DELETE FROM tmmembers WHERE id = ${member.id} AND roleid = ${roleid};`, {
                ...
                if (sqlResult[0].count != 0) {
                    ...
                    for (let i = 0; i < count; i++) {
                        ...
                        if (love_money > 2500) {
                            await connection
                                .query(`UPDATE tmroles SET love_money-2500 love_time = love_time+${1000*60*60*24*14} WHERE partner = ${partner};`, {
                                    type: QueryTypes.UPDATE, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
                                })
                        } else {
...
[7] Совместный онлайн: ${love_online}`)
                                .setFooter({text: `${partner.id} • ${interaction.guild.name}`})
                                .setTimestamp();
                            await logChannel.send({
                                embeds: [EmbedLog],
                            })
