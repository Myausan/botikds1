        ...
        try {
            ...
            if (a_balance < 5000) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(author.user.displayAvatarURL())
                    .setColor(config.colorError)
                    .setDescription(`${member}, что бы вступить в брак, вам нужно купить кольца, их стоимость 5000${emoji}\n\nВаш баланс: ${a_balance}${emoji}`);
                await interaction.reply({
                    embeds: [errorEmbed]
                })
                return
            ...
            const Embed = new EmbedBuilder()
                ...
                .setDescription(`${author}, для покупки колец нужно 5000 ${emoji}, вы уверены что хотите совершить данную операцию?`)
            await interaction.reply({
                embeds: [Embed],
                components: [row],
                fetchReply: true
