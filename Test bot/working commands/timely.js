    ...
    async execute(interaction, connection, lockedCommands) {
        ...
        try {
            ...
            let text = (timely1, timely2, timely3, timely4, timely5) => {
                ...
                let temp = `${tmp.ramka11+tmp.up+tmp.upt+tmp.up+tmp.upt+tmp.up+tmp.upt+tmp.up+tmp.ramka12}\n${tmp.left+timely1+tmp.middle+timely2+tmp.middle+timely3+tmp.middle+timely4+tmp.middle+timely5+tmp.right}\n${tmp.ramka21+tmp.down+tmp.downt+tmp.down+tmp.downt+tmp.down+tmp.downt+tmp.down+tmp.downt+tmp.down+tmp.ramka22}`
                return temp
            }
            await connection
