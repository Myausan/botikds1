...
module.exports = {
	data: new SlashCommandBuilder()
		...
		try {
			...
			await connection
				.query(`UPDATE money SET money = money-${money}, bank = bank+${money} WHERE id = ${author.id};`, {
					type: QueryTypes.UPDATE, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
				})
			const embed = new EmbedBuilder()
				.setTitle("Пополнение счёта")
				.setThumbnail(author.user.displayAvatarURL())
				.setDescription(`${author} вы перевели ${money} ${emoji} в ${config.emojianim}

Теперь у вас на счету:`)
				.addFields(
					{name: `${emoji} Кристаллов:ᅠᅠ`, value: `\`\`\`${balance-money}\`\`\``, inline: true},
					{name: `${config.emojianim} Электрокристаллы:`, value: `\`\`\`${bank+money}\`\`\``, inline: true}
				)
