...
for (const file of playerEventFiles) {
	...
	const event = require(filePath);
	if (event.once) {
		client.once(event.name, (...args) => event.execute(...args, connection, DB));
	} else {
		client.on(event.name, (...args) => event.execute(...args, connection, DB));
	}
}

client.Memory = new DiscordDB("Memory", client); //Памятная память
let player = client.player = new Player(client)
client.Memory.save();
