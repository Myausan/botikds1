...
module.exports = {
	...
	async execute(interaction, connection, DB) {
		...
		try {
			...
			const canvasava3 = Canvas.createCanvas(512, 512);
			const ctxava3 = canvasava2.getContext('2d');
			...
			let imageResponse = await axios.get(member1.displayAvatarURL({extension:'png', size: 512}), {
			...
			ctxava2.drawImage(avatar1, 0, 0, 512, 512)
			...
			//ctxava2.globalCompositeOperation = 'source-in'
			//ctxava2.drawImage(square, 0, 0, 512, 512)
			//ctxava3.fillStyle = '#fff'
			//ctxava3.fillRect(0, 0, 512, 512)
			//ctxava3.globalCompositeOperation = 'source-in'
			//ctxava3.drawImage(square, 512, 512);
			//ctxava2.fillStyle = '#fff'
			/*const temp = canvasava2.height-200
			for (let y = 0; y < temp; y++) {
				const alpha = (temp - y) / temp*0.7;
				ctxava2.globalAlpha = alpha;
				ctxava2.drawImage(avatar1, 0, y, canvas.width, 1, 0, y, canvas.width, 1);
			}
			ctxava3.drawImage(avatar1, 50, 50, 50, 50)
			ctxava1.fillStyle = '#fff'
			ctxava1.fillRect(0, 0, 512, 512)
			ctxava1.globalCompositeOperation = 'source-in'
			ctxava1.drawImage(square, 0, 0, 512, 512)
			ctxava1.drawImage(avatar1, 0, 0, 512, 512)
			ctxava1.globalCompositeOperation = 'source-over';
			for (let y = 0; y < temp; y++) {
				const alpha = (temp - y) / temp*0.7;
				ctxava1.globalAlpha = alpha;
				ctxava1.drawImage(avatar1, 0, y, canvasava1.width, 1, 0, y, canvasava1.width, 1);
			}*/
			...
			function drawCircularProgressBarLine( x, y, radius, color, progress) {
				...
				// Translate the canvas to the center of the progress bar
				ctx.translate(x, y);
			  
				// Rotate the canvas so that the progress bar line starts at the top
			  ...
			  // Example usage
			drawCircularProgressBarLine(327.5, 327.5, 117, '#df3e36', 40);
			ctx.fillStyle = '#fff'
			ctx.font = `38px Inter`;
			ctx.fillText(`${0}`, 517, 551, 82)
			ctx.textAlign = "left";
			...
			*/
			...
			ctx.fillText(`${120}`, 247, 925)//кристаллы
			...
			ctx.fillText(`${'Отсутствует'}`, 849, 897, 309)
