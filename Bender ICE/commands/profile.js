...
module.exports = {
	...
	async execute(interaction, connection, DB) {
		...
		try {
			...
			await connection
            .query(`SELECT COUNT(id) as count FROM money WHERE money>${balance};`, {
                type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
            })
				...
				if (buttonId === 'buttonProfileEditImage') {
					...
					collector1.on('collect', async ButtonInteraction => {
						...
						if (ButtonInteraction.customId === 'selectMenuProfileItems') {
							status = 'editProfile'
							collector1.stop()
							const nameProfile = ButtonInteraction.values[0];
							const Embed = new EmbedBuilder()
								.setTitle('Изменение фона')
								.setColor(config.color)
								.setDescription(`${author}, вы изменили фон профиля на \`${nameProfile}\``);
							await interaction.editReply({
								embeds: [Embed],
								components: [],
								files: []
							...
							await connection
								.query(`UPDATE money SET profile = '${nameProfile}' WHERE id = ${author.id};`, {
							...
							if (ButtonInteraction.customId === 'buttonProfileLeft') {
								page--
							}
							...
							for (let i = page*6; i < max(page*6+6); i++) {
								...
								ctxProfile.drawImage(profile, 0, 0, 1888, 1056)
								ctxProfile.globalCompositeOperation = 'source-over';
								ctx.drawImage(canvasProfile, coords[String(i%6)].profile.x, coords[String(i%6)].profile.y, 547.75, 306.5);
								ctx.drawImage(squareNumber, coords[String(i%6)].number.x, coords[String(i%6)].number.y, 58, 58);
								ctx.fillText(`${i+1}`, coords[String(i%6)].number.x+28, coords[String(i%6)].number.y+42)
								ctxProfile.clearRect(0, 0, ctxProfile.width, ctxProfile.height);
							}
							for (let i = page*6; i < max(page*6+6); i++) {
								selectMenu.addOptions(
