...
[12] Новый баланс ${member}: ${m_balance}`
            ...
            if (member == null) {
                ...
                collector.on('collect', async ButtonInteraction => {
                    ...
                    await connection
                        .query(`SELECT money, jailtime, baneconomy, bypass1, ghost FROM money WHERE id = ${ButtonMember.id}`, {
                            type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
                        })
                        .then((result) => sqlResult = result)
                    if (sqlResult[0] === undefined) {
...
Баланс ${member}: ${m_balance+money-fee} ${emoji}`);
                ...
                await connection
                    .query(`SELECT money, jailtime, bypass, baneconomy, ghost FROM money WHERE id = ${member.id}`, {
                        type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
                })
                    .then((result) => sqlResult = result)
                if (sqlResult[0] === undefined) {
