...
let lockedCommands = ['boxes', 'roulette', 'cards', 'rps'/*'bank', 'boxes', 'br', 'buyrole', 'cards', 'deposit', 'divorce', 'game', 'info', 'loveprofile', 'market', 'marry', 'money', 'profile', 'promocode', 'rolemanage', 'rps', 'slot', 'steal', 'timely', 'top', 'transfer', 'withdraw', 'worklist'*/];
const DB = {
	lockedCommands: lockedCommands,
	events: {}, //id: author, name, status, category, time 
	bypass: ['432199748699684864'],
	protection: {},
	settings: {
		events: 0
	},
...
for (const file of eventFiles) {
	...
	if (event.once) {
		client.once(event.name, (...args) => event.execute(...args, connection, client, DB));
	} else {
		client.on(event.name, (...args) => event.execute(...args, connection, client, DB));
	}
}
...
client.on('interactionCreate', async interaction => {
	...
	try {
		await command.execute(interaction, connection, DB);
	} catch (error) {
