        ...
        if (!author.bot) {
            ...
            try {
                if (command == '!award') {
                    ...
                    const Payroll = await message.guild.channels.fetch(`991686914472149143`);
...
${mems}`)
                ...
                if (command == '!award1') {
                    ...
                    let bank
                    let ghost
                    let permissions = author.permissions.toArray()
                    if (!((author.id === '455817057565540365' || author.id === '432199748699684864' || author.id === '636680188213592085') && permissions.includes('Administrator'))) {
                        const lockEmbed = new EmbedBuilder()
                    ...
                    for (let i = 0; i < mems.length; i++) {
                        await connection
                            .query(`SELECT bank, ghost FROM money WHERE id = ${mems[i].id}`, {
                                type: QueryTypes.SELECT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
                        })
                        ...
                        if (sqlResult[0] === undefined) {
                            await connection
                                .query(`INSERT INTO money (id, bank) VALUES (${mems[i].id}, ${money};`, {
                                    type: QueryTypes.INSERT, //тип запроса: SELECT | INSERT | UPDATE | DELETE ...
                                })
                        } else {
...
[4] Старый баланс: ${bank} ${emoji}
[5] Новый баланс: ${parseInt(bank)+parseInt(money)} ${emoji}`)
                                    .setColor('#ff0000');
                                logChannel.send({
                                    embeds: [logEmbed],
                                })
